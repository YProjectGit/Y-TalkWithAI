...
このワークショップでよく使う枠の目安（2026年8月、アカウントによって違います。表の数字を正とする）:
...
| Live / TTS 向けモデル | モデルごとに別枠 | 3A の声、3B / 4 / 5 の Live |
...
「数秒後に再試行」と書いてあっても、**日次の回数を使い切っていると待ちでは直りません**（リセットは日付が変わってから。タイムゾーンはアカウント側）。
...
上限に達すると、そのプロジェクトの API は止まります。反映まで **約10分** かかることがあり、そのあいだに少し超えることがあります。Live を繋いだままにしないでください。
...
従量課金です。月額の定額ではありません。単価は 100万トークンあたり（2026年8月、Standard。3.6 は 2026年末までの価格）。
...
| Live（3B / 4） | `gemini-3.1-flash-live-preview` | 音声 約 $0.005/分 | 音声 約 $0.018/分 |
