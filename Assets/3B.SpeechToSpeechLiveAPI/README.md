...
SpeechToSpeechのやり取りを、**Gemini Live API**の**ひとつのセッション**で行うアプリケーションです。

3Aのように「文字起こし用」「チャット用」「TTS用」とリクエストを分けず、**WebSocket通信**により音声を双方向に流します。
送っては待つ形ではなくなり、人と話すのに近いテンポになります。

学習のため、接続時のSetupと、送受信される音声と文字のメッセージのチャンク（小さなまとまり）が見えるようになっています。
...
- ### Gemini Live API
  ...
  WebSocketで双方向のストリーミングセッションを張り、音声をやり取りする方法を学びます。
...
**Live API** とは、HTTPの `generateContent` を何回も呼ぶのではなく、**WebSocketで1本のセッションを張り、音声をチャンク（小さなまとまり）で双方向に流す**仕組みです。

このデモではPlay時に、接続とSetupを行い、そのあとPCM（音声データ）を `realtimeInput` で送り、サーバからのPCMとtranscription（文字起こし）を受け取ります。声色はSetupの `speechConfig`（`voiceName`）で一度指定します。

3Aのデモのように「文字起こし用」「チャット用」「TTS用」とリクエストを分けず、1セッションで済むので構成は非常にシンプルですが、今まで使っていた**HTTP通信**ではなく、**WebSocket通信**を行う必要があり、プログラミングの難度は上がります。
...
-　これまでのデモは、インターネット通信で最も一般的な **HTTP**プロトコルを使ったリクエスト／レスポンスの通信でした。クライアントが送り、サーバが答え、そこで1回の通信が終わります。通信の都度、接続の確立やヘッダ（付加情報）のやり取りなどの手続きを行うので、信頼性はありますが、高速なレスポンスを必要とする通信には向きません。

-　**WebSocket** プロトコルは、一度クライアント/サーバーが繋がると、その回線を張ったまま、双方向にメッセージを流し続けられる通信のプロトコルです。一度つながったあとは、HTTPのように毎回大きな手続きを繰り返さないので、やり取りするデータが軽く、高速、低遅延で送受信できます。

-　チャットや通話・同時編集コンテンツ・ライブ通知（株価など）・対戦ゲームなどのようなアプリケーションに使われています。

-　このデモの接続先は次のURLです（キーはクエリに付けます）`wss://generativelanguage.googleapis.com/ws/google.ai.generativelanguage.v1beta.GenerativeService.BidiGenerateContent`
...
-　**ストリーミング** とは、データを全部そろえてから一度に送るのではなく、できた分から小さく分けて流し続けることです。動画の配信などで使われている手法です。
...
中央の送信ログと右の受信ログに出る `+音声chunk` が、流れている音声チャンクの要約です。中央が送ったぶん、右が返ってきたぶんです。
