...
AIからの返答を、文章ではなく**プログラムがそのまま読めるJSON**として受け取るアプリケーションです。
...
**スキーマ**とは、返してほしいJSONの構造をあらかじめ定めたものです。JSONに含めるキー、値の型、入れ子の構造などを指定します。このデモでは、キューブと背景の色を0.0〜1.0の `r` / `g` / `b` で受け取るように指定しています。実際にGemini APIへ送っているスキーマは次のとおりです。
