...
// 2A.SpeechToText の派生デモ。STT だけ whisper.unity（ローカル）に差し替える。
...
public class SpeechToTextWhisper : MonoBehaviour
{
    ...
    public TMP_Text sttRequestText; // 1. Local STT（whisper.unity）
    ...
    IEnumerator Start()
    {
        ...
        if (whisper == null || !whisper.IsLoaded)
        {
            ...
            SetPanelPlaceholder(sttResponseText, "Docs/whisper-unity-setup.md を見て ggml モデルを配置してください。");
        }
        else
        {
    ...
    IEnumerator SetupWhisperCoroutine()
    {
        ...
        if (!File.Exists(modelPath))
        {
            whisperLastError =
                ...
                + "\nDocs/whisper-unity-setup.md を見て配置してください。";
        ...
        if (!whisper.IsLoaded)
        {
            whisperLastError =
                ...
                + "\nコンソールと Docs/whisper-unity-setup.md を確認してください。";
            Debug.LogError("[SpeechToTextWhisper] " + whisperLastError);
        }
    }
        ...
        if (whisper == null || !whisper.IsLoaded)
        {
            ShowError(
                whisperLastError != null
                    ? whisperLastError
                    : "Whisper が初期化されていません。Docs/whisper-unity-setup.md を見てください。",
                sttResponseText);
            return;
        }
    ...
    float[] CopyClipSamples(AudioClip source, int positionSamples)
    {
        ...
        // Whisper はモノラル想定。複数チャネルなら平均して 1ch にする
    ...
    string BuildLocalSttRequestText(int sampleCount, float audioSeconds)
    {
        ...
        sb.AppendLine("engine: whisper.unity WhisperManager");
    ...
    bool TryExtractAssistantText(string responseBody, out string assistantText)
    {
        ...
        {
            Debug.LogError("[SpeechToTextWhisper] JSON 解析失敗: " + e.Message);
            return false;
        }
    ...
    void LoadApiKey()
    {
        ...
        if (!File.Exists(path))
        {
            Debug.LogError("[SpeechToTextWhisper] APIキーファイルがありません: " + path);
            apiKey = null;
            return;
        }
        ...
        if (string.IsNullOrEmpty(apiKey))
        {
            Debug.LogError("[SpeechToTextWhisper] APIキーが空です: " + path);
            apiKey = null;
        }
    }
